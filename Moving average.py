...
def run_strategy(df, leverage=10, commission_rate=0.001):
    ...
    for i in range(200, len(df)):
        ...
        # Safe CPR check using iloc (no .at[] to avoid KeyError)
        try:
            narrow_flag = bool(df["narrow_cpr"].iloc[i])
        except:
            narrow_flag = False

        if position is None and narrow_flag and is_near_sma(current["Close"], df["sma_20"].iloc[i]):
...
st.title("📊 Nifty50 CPR + SMA Strategy Backtest")
...
if st.button("Run Backtest 🚀"):
    ...
    ax[0].legend(); ax[0].set_title("Price & SMA")

    ax[1].plot(result["timestamp"], result["equity"], color="green", label="Equity")
    ax[1].set_title("Equity Curve")
    st.pyplot(fig)

    st.subheader("Trade Log")
    st.dataframe(result[result["signal"] != 0][["timestamp", "signal", "entry_price", "position"]])
